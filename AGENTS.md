...
```
{num}\t{date}\t{company}\t{role}\t{status}\t{score}/5\t{resume_md}\t{final_score}\t{pdf_emoji}\t[{num}](reports/{num}-{slug}-{date}.md)\tjds/{company-role}.md\t{note}
```
...
7. `resume-md` -- root-relative tailored resume Markdown path, e.g. `output/cv-{candidate}-{company}.md`, or blank/`—`
8. `final-score` -- tailored-resume score as `X.X/5`, or blank/`—`/`N/A`
9. `pdf` -- `✅` or `❌`
10. `report` -- markdown link, always written **root-relative**: `[num](reports/...)`
11. `jd` -- root-relative job-description markdown path, e.g. `jds/{company-role}.md` (blank only when no JD could be captured)
12. `notes` -- one-line summary

**Note:** In applications.md, score comes BEFORE status, `resume-md` and `final-score` come before `PDF`, and `JD` comes after `Report`. The merge script handles the score/status swap and rewrites root-relative JD/resume paths to tracker-relative links such as `../jds/{company-role}.md` and `../output/cv-{candidate}-{company}.md`.
